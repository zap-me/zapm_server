...
class Seeds(db.Model):
    ...
    def __init__(self, user, wallet_seed, wallet_address, merchant_rate, customer_rate):
        self.user = user
        self.wallet_seed = wallet_seed
        self.wallet_address = wallet_address
        self.merchant_rate = merchant_rate / 100
        self.customer_rate = customer_rate / 100
...
class SeedsUserModelView(BaseOnlyUserOwnedModelView):
    ...
    def after_model_change(self, form, model, is_created):
        ...
        db.session.query(User).filter(User.id == model.user_id).update({User.customer_rate: model.customer_rate, User.merchant_rate: model.merchant_rate})
        db.session.commit()

    form_columns = ['wallet_seed']
    column_editable_list = ['merchant_rate', 'customer_rate']
