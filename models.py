...
from utils import generate_key, ib4b_response, bankaccount_is_valid, blockchain_transactions, apply_merchant_rate, is_email, generate_random_password, is_address, is_mobile 
...
#    @classmethod
#    def from_seeds_customer_rate(cls, session, Seeds):
#        return session.query(Seeds.customer_rate).filter(Seeds.user_id == cls.id).first()
...
class Seeds(db.Model):
    ...
    def __init__(self, user, wallet_seed):
        self.user = user
...
class Proposal(db.Model):
    ...
    #def __init__(self, merchant, reason):
    ...
    def _format_information_column(view, context, model, name):
        for seed_wallet, in db.session.query(Seeds.wallet_seed).filter(Seeds.user_id == model.id):
            ...
            html = '''
                ...
                rebate address:  <a href="{explorer_url}/address/{rebate_address}/tx">{rebate_address}</a>
                rebate balance:
                <span id="{rebate_address}-balance"></span>
                <script>
                    var xhr = new XMLHttpRequest();
                    var url = "{node_url}/assets/balance/{rebate_address}/{asset_id}";
                    xhr.onreadystatechange = function() {{
                        if (this.readyState == 4 && this.status == 200) {{
                            var json = JSON.parse(this.responseText);
                            var bal = (json.balance / 100).toFixed(2);
                            document.getElementById("{rebate_address}-balance").textContent = bal + ' ZAP';
                        }}
                    }};
                    xhr.open("GET", url, true);
                    xhr.send();
            ...
            '''.format(address=model.wallet_address, node_url=node_url, asset_id=asset_id, explorer_url=explorer_url, bank_account_number=model.banks.first(), rebate_address=rebate_addr.address)
...
class SeedsUserModelView(BaseOnlyUserOwnedModelView):
    ...
    column_list = ('user', 'wallet_seed', 'rebate_address' )
    column_formatters = {'rebate_address': format_rebate_address}
